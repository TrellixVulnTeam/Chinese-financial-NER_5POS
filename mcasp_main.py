...
from mcasp_model import MAINMODEL
import datetime
from Atten_Bilstm import main as ma
...
def train(args):
    ...
    hpara = MAINMODEL.init_hyper_parameters(args)
    tagger = MAINMODEL(word2id, gram2id, gram2count, label_map, hpara, model_path=args.bert_model)
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        if args.local_rank == -1 or torch.distributed.get_rank() == 0:
            ...
            for flag in ['dev', 'test']:
                ...
                output_eval_file = os.path.join(output_model_dir, "results.%s.txt" % flag)#训练好的文件
...
def test(args):
    ...
    tagger = MAINMODEL.load_model(args.eval_model)
