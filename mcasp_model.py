...
from pytorch_pretrained import BertModel as BertModel1
...
from Atten_Bilstm.main import blstm
...
class MAINMODEL(nn.Module):
    ...
    def __init__(self, word2id, gram2id, gram2count, labelmap, hpara, model_path):#增加data_path
        super().__init__()
        ...
        if self.hpara['use_bert']:
            ...
            self.zen_tokenizer = zen.BertTokenizer.from_pretrained("./ERNIE_pretrain", do_lower_case=self.hpara['do_lower_case'])
            ...
            self.ernie = BertModel1.from_pretrained("./ERNIE_pretrain")
        ...
        if self.use_attention:
            ...
            self.classifier = nn.Linear(hidden_size * (2 + self.cat_num), self.num_labels, bias=False)
        else:
            self.multi_attention = None
            self.classifier = nn.Linear(6912, self.num_labels, bias=False)


        self.crf = CRF(self.num_labels, batch_first=True)
        self.blstm =blstm

        print("**** The model used is combined as ****")
        print("use ngram+bert:","Done")
        if self.zen_tokenizer != None:
            print("use ernie:","Done")
        if self.blstm!=None:
            print("use blstm+attention:","Done")
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None,
        ...
        if self.bert is not None:
            ...
            sequence_output1,_=self.ernie(input_ids,attention_mask=attention_mask,output_all_encoded_layers=False)
           # sequence_output1 = self.fc(sequence_output1)
        else:
            raise ValueError()
        ...
        if self.multi_attention is not None:
            ...
            sequence_output = torch.cat([sequence_output, attention_output,sequence_output1], dim=2)


        #sequence_output=self.ernie(sequence_output,output_all_encoded_layers=False)
    ...
    def load_model(cls, model_path):
        ...
        res.load_state_dict(torch.load(os.path.join(model_path, 'pytorch_model.bin'),map_location = torch.device('cpu')))
